...
/**
 * This class peforms the DBScan algorithm on any CSV file containing 
 * points with x, y and z coordinates.
 * 
 * @author Jenny-Carl Kaze
 */
public class DBScan {
    private double eps;
    private int minPts;
    ...
    public DBScan(List<Point3D> listPts){
        eps = 0.0;
        minPts = 0;
        clusters = 0;
    ...
    public void findClusters(){
        for (Point3D point3d : listPts) {
            if (point3d.getLabel() == -1) {
                NearestNeighbors nearN = new NearestNeighbors(listPts);
